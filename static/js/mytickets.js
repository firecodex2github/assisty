...
async function fetchTickets() {
  ...
  try {
    ...
    tickets.forEach((t) => {
      ...
      const pOpen =
        status === "Open" ? "bg-info text-white" : "bg-light text-muted";
      const pProgress =
        status === "In Progress"
          ? "bg-primary text-white"
          : "bg-light text-muted";
      const pResolved =
        status === "Resolved" ? "bg-success text-white" : "bg-light text-muted";
      const pClosed =
        status === "Closed" ? "bg-danger text-white" : "bg-light text-muted";

      // Unread Indicator Logic
      const hasUnread =
        localStorage.getItem(`unread_${t.ticket_no}`) === "true";
      const dotHtml = hasUnread
        ? `<span id="dot-${t.ticket_no}" class="unread-dot"></span>`
        : `<span id="dot-${t.ticket_no}"></span>`;
...
window.openChat = async function (ticketNo) {
  ...
  socket = new WebSocket(
    `${protocol}://${window.location.host}/ws/chat/${ticketNo}/user?token=${token}`,
  );
  ...
  if (inputField) {
    inputField.oninput = () => {
      //hits every time input value changes even one letter type
  ...
  socket.onmessage = (event) => {
    ...
    if (!isChatVisible || activeUserChatTicket !== ticketNo) {
      //Chat window is closed  or Chat is open, but for another ticket
      if (data.sender !== "user") {
...
window.viewIssue = async function (ticketNo) {
  ...
  try {
    ...
    if (t.attachments) {
      t.attachments.forEach((url) => {
        if (url.toLowerCase().endsWith(".pdf"))
          pdfsHtml += `<a href="${url}" target="_blank" class="btn btn-sm btn-outline-danger mb-2 p-2 w-auto d-block"><i class="bi bi-file-earmark-pdf"></i> PDF</a>`;
        else
          imagesHtml += `<img src="${url}" class="attachment-preview me-2 mb-2" onclick="window.open('${url}', '_blank')">`;
      });
    }
    detailsBody.innerHTML = `
            <div class="row g-3">
    ...
    detailsBody.innerHTML = "Error loading.";
  }
};
...
window.closeIssueModal = () =>
  (document.getElementById("viewIssuePopup").style.display = "none");
...
window.logout = async function () {
  ...
  if (refreshToken) {
    try {
      await fetch(
        `/user/logout?refresh_token=${encodeURIComponent(refreshToken)}`,
        {
          method: "POST",
